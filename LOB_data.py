...
class LOB_data:
    def __init__(self, path_folder, label_message_file, label_ob_file, tick_size):
        """
        Class that allows to load and clean a LOBSTER data set.
    ...
    def load_LOB_data(self, verbose = True, message_file_with_extra_column = False):
        ...
        We recall that the k-th row in the "message file" describes the event causing the change in the LOB from line k-1 to line k in the "orderbook file".
        ...
        - 2nd entry: event type (1: submission of LO, 2: partial deletion of LO,  3: tot deletion of LO, 4: execution of a visible LO,  5: execution of a hidden LO, 6: auction trade, 7: trading halt indicator)
        - 3rd entry: order ID
        ...
        self.ob_file = pd.read_csv(self.path_folder + self.label_ob_file, header = None)
        self.n_levels = len(self.ob_file.columns)//4
    ...
    def clean_trading_halts(self, verbose = True):
        """
        Function which cleans the data from trading halts.
        When trading halts occur, a message of type '7' is written into the "message file". 
    ...
    def clean_opening_closing_auctions(self, verbose = True):
        ...
        Sometimes, messages related to the daily opening and closing auction are included in the LOBSTER file (especially during days when trading stops earlier, e.g., before holidays).
        We remove such observations (opening auctions are identified as messages with type == 6 and ID == -1, and closing auctions can be identified as messages with type == 6 and ID == -2)
    ...
    def handle_splitted_lo_executions(self, verbose = True):
        ...
        if verbose == True:
            print('Out of %d events, %d are associated to unique times'%(len(self.message_file), len(events_times)))
